...
| 2026-02-25 | Admin backend: role foundation, layout, incidents table, business approvals, contact messages | `prisma/schema.prisma`, `src/lib/auth-admin.ts`, `src/app/admin/*`, `src/components/ui/table.tsx` | User.role enum, requireAdmin guard, TanStack Table for incidents | `bun run build` | pending | pending |
